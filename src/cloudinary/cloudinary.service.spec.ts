...
function testingModuleWithCloudinaryOptions(options: CloudinaryModuleOptions) {
...
describe('CloudinaryService', () => {
  ...
  beforeEach(async () => {
    resetMocks();
    const module: TestingModule = await testingModuleWithCloudinaryOptions({
  ...
  describe('folder_root option', () => {
    ...
    beforeEach(async () => {
      resetMocks();
      const module = await testingModuleWithCloudinaryOptions({
        cloud_name: 'test_cloud',
        api_key: 'test_key',
        api_secret: 'test_secret',
        folder_root: 'my-app',
  ...
  describe('uploadMany', () => {
    it('returns distinct results when every upload succeeds', async () => {
      let call = 0;
      cloudinaryMocks.uploadStream.mockImplementation(
        (_opts, cb: UploadStreamCallback) => {
          const s = new PassThrough();
          queueMicrotask(() => {
            call++;
            cb(null, {
              secure_url: `https://u${call}`,
              public_id: `folder/f${call}`,
            });
          });
          return s;
        },
